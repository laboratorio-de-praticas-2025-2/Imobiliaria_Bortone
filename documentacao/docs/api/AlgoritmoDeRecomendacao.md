...
recomendacoesRoutes.post(
  "/recomendacao_imovel",
  validacaoRecomendacaoImovel,
  createRecomendacaoImovel
);
...
1. **_Consulta de Dados:_** Busca no modelo `Imovel` os detalhes completos (tipo, cidade, estado, preco) dos imóveis visitados. O operador Sequelize.Op.in permite buscar múltiplos IDs em uma única consulta eficiente.

2. **_Análise de Frequência (lodash):_** Utiliza a biblioteca lodash para analisar o `imoveisReferencia` e identificar os valores mais comuns para tipo, cidade e estado. O fluxo é:
...
3. **_Cálculo da Faixa de Preço:_** Calcula o preço médio dos imóveis visitados e define uma faixa de tolerância de 20% para cima e para baixo (80% a 120% do preço médio). Isso cria uma faixa flexível para as recomendações.
...
1. **_Identifica os Populares:_** Primeiro, ela consulta a tabela recomendacao_imovel para encontrar os 20 imóveis que foram mais visitados no total. A consulta usa COUNT e GROUP BY para somar as visitas por `imovel_id` e ORDER BY para listar os mais populares primeiro.

2. **_Filtra e Retorna Dados Completos:_** Em seguida, a função extrai apenas os ids desses imóveis populares. Com essa lista de ids, ela faz uma nova consulta no modelo `Imovel`. Essa segunda consulta garante que apenas imóveis que estão `disponivel` sejam retornados, oferecendo uma lista de recomendações que o usuário pode, de fato, alugar ou comprar.
...
- **Descrição:** Esta função implementa o fluxo completo da lógica de recomendação de imóveis para um usuário específico. Ela começa analisando o histórico de visitas do usuário e, dependendo desse histórico, segue um rigoroso sistema de tentativas de busca (fallbacks) para garantir que uma lista de recomendações relevante seja sempre entregue.

- **Fluxo de Busca e Fallbacks:** A função segue um fluxo em cascata para garantir que, mesmo com um histórico limitado, o usuário receba resultados:

1. **_Pré-Verificação (Fallback Inicial):_:** Se o usuário não tiver NENHUM histórico de visitas (`imoveisVisitados` está vazio), a função aciona imediatamente o primeiro fallback, retornando os imóveis mais populares do sistema (`getImoveisPopulares`).

2. **_Busca Estrita por Preferência (Tentativa Principal):_** Se houver histórico, a função infere as preferências (tipo, cidade, estado e faixa de preço) e tenta encontrar imóveis que correspondam a todos esses filtros.

3. **_Fallback para Busca Expandida (Segunda Tentativa):_** Caso a Busca Estrita não retorne resultados `(_.isEmpty(imoveisRecomendados))`, o algoritmo relaxa os critérios. Ele remove os filtros de localização (cidade/estado) e preço, mantendo apenas o tipo preferido do imóvel.

4. **_Fallback Final para Populares (Terceira Tentativa):_** Se a Busca Expandida ainda não produzir resultados, a função aciona o fallback final, chamando getImoveisPopulares (excluindo os imóveis já visitados) para garantir que o array de resposta da API não seja vazio.

- **Pós-Processamento e Coerência de Dados:** Após todas as tentativas de busca, a lista final de imóveis passa por uma etapa de limpeza e formatação `(imoveisTratados.map)`. O objetivo é garantir que a chave das comodidades na resposta JSON corresponda ao tipo de imóvel.

  - O Sequelize, por padrão, retorna os dados de comodidades sempre sob a chave casa.

  - Nesta etapa, o código renomeia dinamicamente essa chave casa para apartamento ou terreno quando necessário.

  - Essa ação assegura que a estrutura de dados final seja logicamente coerente para o usuário, apresentando o objeto de comodidades com o nome exato do tipo do imóvel.
...
export const getRecomendacoes = async (req, res) => {
  ...
  try {
    const recomendacoes =
      await recomendacaoImovelService.getRecomendacoesByUserId(usuario_id);
    res.status(200).json({
    ...
    res
      .status(500)
      .json({ error: "Erro ao gerar recomendações: " + err.message });
  }
};
```
	...
	"data": [
		{
			...
			"preco": "190000.00",
			"area": 65,
			"descricao": "Apartamento compacto, bem localizado",
			"data_cadastro": "2025-08-16",
			"murado": 0,
			"latitude": "-24.4881000",
			"longitude": "-47.8402000",
			"usuario_id": 1,
			...
			"imagem_imovel": [
				{
					"id": 54,
					"url_imagem": "8d501acf45a17771119f1ce8f3f9ecaf.png"
				},
				{
					"id": 55,
					"url_imagem": "4c52f5a2b25535b01cfeba2fbbc5731b.png"
				}
			],
			"apartamento": {
				"quartos": 1,

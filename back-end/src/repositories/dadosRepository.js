...
async function executeQuery(query, errorMessage, replacements = {}) {
  try {
...
export async function buscarDadosParaRelatorio(secoes, data_inicio, data_fim) {
  try {
    let dadosRelatorio = {};

    if (secoes.includes(REPORTS_SECOES.SUMARIO_EXECUTIVO)) {
      dadosRelatorio.sumarioExecutivo = await obterSumarioExecutivo(
        data_inicio,
        data_fim
      );
    }
    if (secoes.includes(REPORTS_SECOES.JORNADA_CLIENTE)) {
      dadosRelatorio.jornadaCliente = await obterDadosJornadaCliente(
        data_inicio,
        data_fim
      );
    }
    if (secoes.includes(REPORTS_SECOES.ANALISE_ESTOQUE)) {
      dadosRelatorio.analiseEstoque = await obterDadosAnaliseEstoque();
    }
    if (secoes.includes(REPORTS_SECOES.DESEMPENHO_VENDAS)) {
      dadosRelatorio.desempenhoVendas = await obterDadosDesempenhoVendas(
        data_inicio,
        data_fim
      );
    }
    if (secoes.includes(REPORTS_SECOES.DESEMPENHO_LOCACOES)) {
      dadosRelatorio.desempenhoLocacoes = await obterDadosDesempenhoLocacoes(
        data_inicio,
        data_fim
      );
    }
...
export async function obterSumarioExecutivo(data_inicio, data_fim) {
  ...
  const [
    ...
    executeQuery(QUERY_QUANTIDADE_TOTAL_VENDAS, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => res[0]?.total || 0),
    executeQuery(QUERY_VALOR_TOTAL_VENDAS, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => res[0]?.valor_total || 0),
    executeQuery(QUERY_AGENDAMENTOS_NOVOS, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => res[0]?.total || 0),
    executeQuery(QUERY_NOVOS_USUARIOS_COM_AGENDAMENTO, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
...
export async function obterDadosAnaliseEstoque() {
  ...
  const [
    ...
    executeQuery(QUERY_ESTATISTICAS_IMOVEIS, ERROR_MESSAGE),
    executeQuery(QUERY_DISTRIBUICAO_IMOVEIS_PRECO, ERROR_MESSAGE),
    executeQuery(QUERY_DISTRIBUICAO_IMOVEIS_TIPO, ERROR_MESSAGE),
    executeQuery(QUERY_TABELA_IMOVEIS_ACESSOS, ERROR_MESSAGE),
  ]);
...
export async function obterDadosJornadaCliente(data_inicio, data_fim) {
  ...
  const [
    ...
    executeQuery(QUERY_TOTAL_USUARIOS_CADASTRADOS, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => res[0].total || 0),
    executeQuery(QUERY_ESTATISTICAS_AGENDAMENTO, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => res[0] || {}),
    executeQuery(QUERY_EVOLUCAO_MENSAL_AGENDAMENTOS_E_USUARIOS, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => Object.values(res[0])),
    executeQuery(QUERY_TABELA_AGENDAMENTOS, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
...
export async function obterDadosDesempenhoVendas(data_inicio, data_fim) {
  ...
  const [
    ...
    executeQuery(QUERY_TOTAL, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }),
    executeQuery(QUERY_DISTRIBUICAO_TIPO, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => Object.values(res[0])),
    executeQuery(QUERY_EVOLUCAO_MENSAL, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }).then((res) => Object.values(res[0])),
    executeQuery(QUERY_REGISTRO_DADOS, ERROR_MESSAGE, {
      data_inicio,
      data_fim,
    }),
  ]);
...
export async function obterDadosDesempenhoLocacoes(data_inicio, data_fim) {

// src/services/simuladorService.js
...
exports.calcularSimulacao = function(dados) {
  const { tipo, valorImovel, entrada, parcelas, modalidade } = dados;
  ...
  if (tipo === 'imovel' || tipo === 'IMOVEL') {
    if (parcelasNum <= 0) {
      return { sucesso: false, erro: 'Parcelas deve ser maior que 0' };
    }
    if (parcelasNum > 420) {
      return { sucesso: false, erro: 'Parcelas não pode ser maior que 420' };
    }
    if (entradaNum > valor) {
      return { sucesso: false, erro: 'A entrada não pode ser maior que o valor do imóvel.' };
    }
  ...
  if (tipo === 'terreno' || tipo === 'TERRENO') {
    ...
    if (entradaNum > valor) {
      return { sucesso: false, erro: 'A entrada não pode ser maior que o valor do terreno.' };
    }
    const principal = Math.max(valor - entradaNum, 0);
  ...
  // Caso o tipo da simulação seja inválido
  return { sucesso: false, erro: 'Tipo de simulação inválido.' };
};
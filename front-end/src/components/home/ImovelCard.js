  ...
  return (
    <Link href={`/imoveis/${imovel.id}`}>
      <div className="sm:p-3 bg-white sm:bg-[#DEE1F0] flex flex-col gap-2 align-middle rounded-xl sm:shadow-none shadow-lg">
            ...
            {(imovel.tipo == "Casa" || imovel.tipo == "Apartamento") && (
              <>
                <Flex gap="small" className="text-[var(--primary)]">
                  ...
                  <span>
                    {imovel.banheiros ||
                      imovel.casa?.banheiros ||
                      imovel.apartamento?.banheiros ||
                      0}
                  </span>
                </Flex>
                <Flex gap="small" className="text-[var(--primary)]">
        ...
        >
          <Link
            href={`/agendamento/${imovel.id}`}
            className="w-full"
            onClick={(e) => {
              // evita que o clique suba pro Link do imóvel
              e.stopPropagation();
            }}
          >
            <Button
          ...
          <Button
            ...
            onClick={(e) => {
              // impede a navegação pra página do imóvel
              e.stopPropagation();
              // aqui você pode abrir modal/ação de mensagem
            }}
          >
